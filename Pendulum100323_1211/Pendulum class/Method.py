...
class method:
    ...
    def a_double(self, theta, omega):
        a1 = ((self._g*(self._M[0]+self._M[1])*np.sin(theta[0]) + self._M[1]*self._g*np.sin(theta[0] - 2*theta[1]) -
                                 2*np.sin(theta[0] - theta[1])*self._M[1]*((omega[1]**2)*self._L[1] + 
                                                                                       (omega[0]**2)*self._L[0]*np.cos(theta[0] - theta[1])))/
                                 (self._L[0]*(2*self._M[0] + self._M[1] - self._M[1]*np.cos(2*theta[0] - 2*theta[1]))))
        a2 = ((2*np.sin(theta[0] - theta[1])*((omega[0]**2)*self._L[0]*(self._M[0] + self._M[1])- self._g*(self._M[0] + self._M[1])*np.cos(theta[0]) + 
                                                                    (omega[1]**2)*self._L[1]*self._M[1]*np.cos(theta[0] - theta[1])))/
                                                                    (self._L[1]*(2*self._M[0] + self._M[1] - self._M[1]*np.cos(2*theta[0] - 2*theta[1]))))

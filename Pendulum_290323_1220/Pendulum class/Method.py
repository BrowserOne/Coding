"""
Program to calculate next step in pendulum motion. Script is divided into simple pendulum and double pendulum calculation for Forward Euler, Euler Cromer and Runge Kutta integration methods.
...
class method:
    ...
    def euler_cromer_double(self, dt):
        ...
        --------
            ...
            self._omega: List containing next step in angular velocity of pendulum system
        """
        self._omega = self._omega + self.a_double(self._theta,self._omega)*dt
        self._theta = self._theta + self._omega*dt
        return self._theta, self._omega
    ...
    def runge_kutta_simpleO2(self, dt):
        ...
        summing a weighted sum of 3/4 step with 1/4 step.
        ...
        omega_next = self._omega + ((1/4)*u1 + (3/4)*u2)*dt
        theta_next = self._theta + ((1/4)*k1 + (3/4)*k2)*dt
